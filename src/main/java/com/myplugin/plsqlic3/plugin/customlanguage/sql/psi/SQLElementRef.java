...
public abstract class SQLElementRef extends PsiReferenceBase<SQLIdentifierPSINode> {
	public SQLElementRef(@NotNull SQLIdentifierPSINode element) {
		/** WARNING: You must send up the text range or you get this error:
		 * "Cannot find manipulator for PsiElement(ID) in com.myplugin.plsqlic3.plugin.customlanguage.sample.SampleElementRef"...
...
//		System.out.println(getClass().getSimpleName()+".isReferenceTo("+refName+"->"+def.getText()+")");
		...
		if ( def instanceof SQLIdentifierPSINode && isDefSubtree(def.getParent()) ) {
			def = def.getParent();
		}
		if ( isDefSubtree(def) ) {
